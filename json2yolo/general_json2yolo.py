...
def extract_coco_categories(categories):
    return {c["id"] - 1: c["name"] for c in categories}
...
def convert_coco_json(
    ...
    split_names = []
    classes = None
    for json_file in sorted(Path(src_annotations_dir).resolve().glob("*.json")):
        split_name = json_file.stem.replace("instances_", "")
        split_names.append(split_name)
        annotation_out_dir = Path(save_dir) / "labels" / split_name
        annotation_out_dir.mkdir()
        image_out_dir = Path(save_dir) / "images" / split_name
        image_out_dir.mkdir()

        with open(json_file) as f:
            data = json.load(f)

        new_classes = extract_coco_categories(data["categories"])
        if classes and new_classes != classes:
            raise Exception("category mismatch between splits")
        classes = new_classes
